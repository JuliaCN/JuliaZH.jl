...
```
在Julia里，函数是一个将多个参数的值组成的元组（tuple）映射到返回值的对象。函数并不是纯数学函数， 因为函数可以改变或者被程序的全局状态所改变。Julia里函数的基本语法定义如下：
...
```
在Julia里有另一种更简洁的函数定义方式。上面展示的传统的函数声明语法和下面紧凑的“赋值形式”是等价的：
...
```
在赋值形式中，函数的主体必须是一个单一的表达式，尽管它可以是一个复合表达式（参阅[Compound Expressions](@ref man-compound-expressions)）。简短的函数定义在Julia里很常见。这种简短的函数语法相当符合惯用用法，很大程度上减少了输入和视觉的噪声影响。
...
```
就像被用作参数一样，Unicode字符也能够被作为函数名使用：
...
```
Julia函数参数遵循“pass-by-sharing”惯例，值传递到函数的时候不会被复制。函数参数本身充当新变量的**绑定（binding）**（能引用值的新地址），但是引用值和传过去的值是一样的。在一个函数内对可变值（如`数组`）做出的修改对于调用者是可见的。Scheme，大多数的Lisp方言，Python，Ruby和Perl，及其他动态语言都能找到这一相同的行为。
